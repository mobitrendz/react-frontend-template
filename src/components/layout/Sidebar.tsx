...
const Sidebar = ({ userRole, userName, onLogout, activeView, onViewChange, isOpen, onClose }: SidebarProps) => {
      ...
      <aside className={sidebarClasses}>
        ...
        <nav className="flex-1 py-6 px-3 space-y-1 overflow-y-auto">
          {filteredItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon className={`w-5 h-5 shrink-0 ${isActive ? 'text-white' : 'text-[var(--text-dim)] group-hover:text-[var(--accent)]'}`} />
                {(!isCollapsed || isOpen) && <span className="font-medium">{item.name}</span>}
              </Link>
            )
          })}
        </nav>
        ...
        <div className="p-3 border-t border-[var(--sidebar-border)] space-y-1">
          ...
          >
            ...
            {(!isCollapsed || isOpen) && <span className="font-medium text-sm">Sign Out</span>}
          </button>
          ...
          {(!isCollapsed || isOpen) && (
            <div className="mt-4 px-3 py-3 bg-[var(--accent-bg)] rounded-2xl flex items-center gap-3">
              ...
              <div className="flex flex-col min-w-0">
                <span className="text-xs font-bold text-[var(--text-h)] truncate">{userName}</span>
                <span className="text-[10px] text-[var(--accent)] uppercase font-black tracking-wider">{userRole}</span>
              </div>
            </div>
          )}
        </div>
